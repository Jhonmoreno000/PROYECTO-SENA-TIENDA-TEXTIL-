...
import { staggerItems, EASES, microPress, microRelease } from '../utilidades/animations';
...
export default function Carrito() {
    const { cartItems, updateQuantity, removeFromCart, clearCart,
        getCartItemCount, appliedCoupon, setAppliedCoupon, getOrderCalculations } = useCarrito();
    const { coupons } = useMetricas();
    ...
    useGSAP(() => {
        if (listRef.current) {
            const items = listRef.current.querySelectorAll('[datos-animate]');
            if (items.length > 0) {
    ...
    if (cartItems.length === 0) {
        return (
            <div className="min-h-screen flex flex-col">
                <Encabezado />
                <PaginaAnimada className="flex-1 flex items-center justify-center section-container">
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-slate-900">
            ...
            <PaginaAnimada className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
