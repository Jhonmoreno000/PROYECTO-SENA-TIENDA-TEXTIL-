...
function AnalisisDevoluciones() {
    const { orders, bugReports, users } = useMetricas();
    ...
    const returnsBySeller = users.filter(u => u.role === 'vendedor' || u.role === 'vendedor').map(vendedor => {
        const sr = bugReports.filter(r => r.sellerId === vendedor.id);
        const so = orders.filter(o => o.sellerId === vendedor.id);
        return { sellerId: vendedor.id, sellerName: vendedor.name, count: sr.length, rate: so.length > 0 ? (sr.length / so.length) * 100 : 0 };
                        ...
                        <div className={`chart-panel ${glassCard} p-6`}>
                            ...
                            {returnsByTypeArray.length > 0 ? (
                                <ResponsiveContainer width="100%" height={300}>
                                    <PieChart>
                                        <Pie datos={returnsByTypeArray} cx="50%" cy="50%" outerRadius={90} dataKey="count" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                        ...
                        <div className={`chart-panel ${glassCard} p-6`}>
                            ...
                            {returnsBySeller.length > 0 ? (
                                <div className="space-y-5">
                                    {returnsBySeller.slice(0, 5).map((vendedor, i) => (
                                        <div key={vendedor.sellerId} className="flex items-center gap-4 group hover:bg-rose-50 dark:hover:bg-rose-900/20 p-2 -mx-2 rounded-xl transition-colors">
                                            ...
                                            <div className="flex-1">
                                                <p className="font-bold text-slate-900 dark:text-white mb-1.5">{vendedor.sellerName}</p>
                                                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                                    <div className="bg-rose-500 h-full rounded-full" style={{ width: `${Math.min((vendedor.count / (returnsBySeller[0]?.count || 1)) * 100, 100)}%` }} />
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-lg font-black text-rose-600 dark:text-rose-400">{vendedor.count}</p>
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">{vendedor.rate.toFixed(1)}%</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
...
export default AnalisisDevoluciones;
