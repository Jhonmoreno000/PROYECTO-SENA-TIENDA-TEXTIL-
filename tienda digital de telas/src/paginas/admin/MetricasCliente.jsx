...
import { getAllClientsMetrics, segmentClients } from '../../utilidades/metricsUtils';
...
function MetricasCliente() {
    const { users, orders } = useMetricas();
    const clients = users.filter(u => u.role === 'cliente' || u.role === 'cliente');
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {topClients.map((cliente, i) => (
                                        <tr key={cliente.id} className="hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-9 h-9 rounded-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 flex items-center justify-center font-black text-indigo-600 dark:text-indigo-400 text-sm">{cliente.name.charAt(0)}</div>
                                                    <div>
                                                        <p className="font-bold text-slate-900 dark:text-white group-hover:text-[#f97316] transition-colors">{cliente.name}</p>
                                                        <p className="text-xs text-slate-400 dark:text-slate-500">{cliente.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 font-black text-slate-900 dark:text-white">{formatCurrency(cliente.metrics.totalSpent)}</td>
                                            <td className="px-6 py-4"><span className="px-3 py-1.5 rounded-xl text-xs font-black bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-400">{cliente.metrics.totalPurchases} ped.</span></td>
                                            <td className="px-6 py-4 font-mono text-sm font-bold text-slate-700 dark:text-slate-300">{formatCurrency(cliente.metrics.averageOrderValue)}</td>
                                            <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">{cliente.metrics.purchaseFrequency > 0 ? `Cada ${Math.round(cliente.metrics.purchaseFrequency)}d` : 'Primera'}</td>
                                            <td className="px-6 py-4 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{cliente.metrics.lastPurchaseDate ? new Date(cliente.metrics.lastPurchaseDate).toLocaleDateString('es-CO') : 'N/A'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
