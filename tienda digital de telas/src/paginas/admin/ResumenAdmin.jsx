/**
 * ResumenAdmin.jsx — ERP Dashboard (D&D Textil)
 *
 * Panel de administración conectado al backend real de PostgreSQL.
 * Consume datos de ContextoMetricas (salesData, recentActivity, orders,
...
import ErpMetricCard from '../../componentes/dashboard/ErpMetricCard';
import ErpAreaChart from '../../componentes/dashboard/ErpAreaChart';
import NotificationFeed from '../../componentes/dashboard/NotificationFeed';
...
export default function ResumenAdmin() {
    ...
    const totalClients = useMemo(() =>
        users.filter(u => u.role === 'cliente' || u.role === 'cliente').length,
    [users]);
    ...
    const salesChart = useMemo(() => {
        ...
        if (erpSalesMetrics?.length > 0) {
            const datos = filters.dateRange === '7d' ? erpSalesMetrics.slice(-7) : erpSalesMetrics;
            return datos.map(s => ({
        ...
        const datos = filters.dateRange === '7d' ? salesData.slice(-7) : salesData.slice(-30);
        return datos.map(s => ({
    ...
    const topSellers = useMemo(() => {
        ...
        orders.forEach(o => {
            const id = o.sellerId;
            if (!id) return;
            const vendedor = users.find(u => u.id === id || String(u.id) === String(id));
            if (!sellerMap[id]) {
                sellerMap[id] = { id, name: vendedor?.name || `Vendedor #${id}`, orders: 0, sales: 0 };
            }
    ...
    if (loading) {
        return (
            <LayoutPanel title="ERP Dashboard" links={enlacesAdmin}>
                <PaginaAnimada>
    ...
    return (
        <LayoutPanel title="ERP Intelligence" links={enlacesAdmin}>
            <PaginaAnimada>
                        ...
                        {topSellers.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody>
                                        {topSellers.map((vendedor) => (
                                            <tr key={vendedor.id} className="border-b border-gray-50 dark:border-slate-800/50 last:border-0 hover:bg-gray-50 dark:hover:bg-slate-800/20">
                                                <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{vendedor.name}</td>
                                                <td className="px-4 py-3 text-gray-500">{vendedor.orders}</td>
                                                <td className="px-4 py-3 font-medium">{formatCurrency(vendedor.sales)}</td>
