...
function RendimientoVendedores() {
    const { users, orders, updateSellerCommission, toggleSellerSuspension } = useMetricas();
    ...
    const sellersRealBI = useMemo(() => {
        ...
        return sellers.map((vendedor) => {
            const sellerOrders = orders.filter(o => String(o.sellerId) === String(vendedor.id));
            ...
            return {
                ...
                biMetrics: {
                    grossSales,
                    profitMargin: vendedor.commissionRate || 15,
    ...
    const handleToggleSuspension = (vendedor) => {
        if (vendedor.suspended) {
            toggleSellerSuspension(vendedor.id);
            setShowSuspendModal(null);
        } else {
            setShowSuspendModal(vendedor);
        }
    };
                ...
                <div className="relative z-10">
                    ...
                    <div className={`chart-container ${glassCard} p-6 mb-8`}>
                        ...
                        <ResponsiveContainer width="100%" height={350}>
                            <ComposedChart datos={chartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {sellersRealBI.map(vendedor => (
                                        <tr key={vendedor.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${vendedor.suspended ? 'opacity-40 grayscale' : ''}`}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-slate-900 dark:text-white">{vendedor.name}</p>
                                                        <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{vendedor.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <p className="font-bold text-slate-900 dark:text-white text-base">{formatCurrency(vendedor.biMetrics.avgMonthlySales)}</p>
                                                <p className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500">Total: {formatCurrency(vendedor.biMetrics.grossSales)}</p>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="inline-flex items-center justify-center px-3 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-black text-xs">
                                                    {vendedor.biMetrics.conversionRate.toFixed(1)}%
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 font-bold text-emerald-600 dark:text-emerald-400 text-base">
                                                {vendedor.biMetrics.profitMargin.toFixed(1)}%
                                            </td>
                                            <td className="px-6 py-4">
                                                {editingCommission === vendedor.id ? (
                                                    <input
                                                        type="number"
                                                        value={tempCommission}
                                                        onChange={(e) => setTempCommission(e.target.value)}
                                                        onBlur={() => handleUpdateCommission(vendedor.id)}
                                                        onKeyPress={(e) => { if (e.key === 'Enter') handleUpdateCommission(vendedor.id); }}
                                                        className={`w-20 px-3 py-1.5 text-right font-mono ${cleanInput}`}
                                                        autoFocus
                                                    />
                                                ) : (
                                                    <button
                                                        onClick={() => { setEditingCommission(vendedor.id); setTempCommission(vendedor.commissionRate?.toString() || '0'); }}
                                            ...
                                            <td className="px-6 py-4">
                                                <button
                                                    onClick={() => handleToggleSuspension(vendedor)}
                                                    className={`px-4 py-2 text-xs font-black uppercase tracking-widest rounded-xl transition-all border-2 ${vendedor.suspended
                                                    ...
                                                    title={vendedor.suspended ? 'Reactivar' : 'Suspender'}
                                                >
                                                    {vendedor.suspended ? <><ToggleLeft className="inline w-4 h-4 mr-1" />Reactivar</> : <><ToggleRight className="inline w-4 h-4 mr-1" />Suspender</>}
                                                </button>
                                            </td>
                                        </tr>
                                    ))}
...
export default RendimientoVendedores;
