...
function buildActivityFromData(orders, users, bugReports, products) {
    ...
    users.slice(0, 10).forEach(u => events.push({ id: `user-${u.id}`, type: 'user', userName: u.name, action: `se registró como ${(u.role === 'vendedor' || u.role === 'vendedor') ? 'vendedor' : 'cliente'}`, amount: null, time: u.createdAt ? new Date(u.createdAt).toLocaleString('es-CO') : 'Reciente', _date: new Date(u.createdAt || 0) }));
...
function ActividadAdmin() {
    const { users, orders, bugReports, recentActivity, refreshData } = useMetricas();
