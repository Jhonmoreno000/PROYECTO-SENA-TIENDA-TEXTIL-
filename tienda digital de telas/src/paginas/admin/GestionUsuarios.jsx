...
const ROLE_LABELS = { admin: 'Administrador', administrador: 'Administrador', vendedor: 'Vendedor', cliente: 'Cliente' };

function GestionUsuarios() {
    const { users, updateUserRole, toggleUserActive } = useMetricas();
    ...
    const filteredUsers = users.filter(u => {
        ...
        const userRole = (u.role || u.rol || 'cliente');
        const mr = activeTab === 'all' || userRole === activeTab || (activeTab === 'admin' && (userRole === 'admin' || userRole === 'administrador')) || (activeTab === 'vendedor' && (userRole === 'vendedor' || userRole === 'vendedor')) || (activeTab === 'cliente' && (userRole === 'cliente' || userRole === 'cliente'));
        return ms && mr;
    });
    ...
    const sellerCount = users.filter(u => u.role === 'vendedor' || u.role === 'vendedor').length;
    const clientCount = users.filter(u => u.role === 'cliente' || u.role === 'cliente').length;

    const TABS = [
        { id: 'all', label: 'Todos', count: totalCount },
        { id: 'admin', label: 'Admins', count: adminCount },
        { id: 'vendedor', label: 'Vendedores', count: sellerCount },
        { id: 'cliente', label: 'Clientes', count: clientCount },
    ];
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-3 shrink-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <select value={user.role || user.rol || 'cliente'} onChange={e => handleRoleChange(user.id, e.target.value)} className={`border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 px-3 py-2 outline-none focus:border-indigo-300 cursor-pointer`}>
