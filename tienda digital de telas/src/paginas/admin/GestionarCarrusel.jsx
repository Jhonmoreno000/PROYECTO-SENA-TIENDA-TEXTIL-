...
function GestionarCarrusel() {
    ...
    useEffect(() => {
        fetch(getApiUrl('/api/carrusel/todos'))
            ...
            .then(datos => { if (datos) setCarouselSlides(datos.map(normalizarSlide)); })
    ...
    useEffect(() => {
        fetch(getApiUrl('/api/secciones-inicio'))
            .then(res => res.ok ? res.json() : null)
            .then(datos => { if (datos) setHomeSections(datos); })
                    ...
                    <div className={`${glassCard} overflow-hidden mt-8`}>
                        <div className="p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Los títulos y subtítulos que se muestran en el Inicio y en el Catálogo. Se guardan en la base de datos.</p>
                            </div>
                        </div>
                        <div className="divide-y divide-slate-200 dark:divide-slate-700">
...
export default GestionarCarrusel;