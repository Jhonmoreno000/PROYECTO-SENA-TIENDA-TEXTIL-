...
import { Encabezado, Portada, Carrusel, ProductosDestacados, Beneficios, PieDePagina, PaginaAnimada, SeccionInicio } from '../componentes';
...
function Inicio() {
    ...
    useEffect(() => {
        ...
        fetch(getApiUrl('/api/secciones-inicio'))
            ...
            .then(datos => { if (datos && datos.length > 0) setHomeSections(datos); })
            .catch(() => {
    ...
    return (
        <div className="min-h-screen">
            ...
            <main>
                <PaginaAnimada>
                    {isVisible('hero') && <Portada />}
                    {isVisible('carousel') && <Carrusel />}
                    ...
                    {isVisible('featured') && <ProductosDestacados />}
                    {isVisible('benefits') && <Beneficios />}
                </PaginaAnimada>
            </main>
            <PieDePagina />
        </div>
