...
import { MdInventory, MdFileDownload, MdVisibility, MdFilterList, MdCalendarToday } from 'react-icons/md';
...
function OrderHistory() {
    const { orders, products } = useMetrics();
    const { user } = useAuth();
            ...
            {clientOrders.length === 0 ? (
                ...
                <div className="space-y-4">
                    {clientOrders.map(order => (
                        <div key={order.id} className="card shadow-sm border border-gray-100 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md overflow-hidden hover:shadow-lg transition-shadow">
                            <div className="p-6">
                                        ...
                                        {(order.productIds ? order.productIds.slice(0, 3) : [null, null, null].slice(0, order.items || 1)).map((id, i) => {
                                            ...
                                            return (
                                                <img
                                                    ...
                                                    className="w-12 h-12 rounded-lg object-cover border-2 border-white dark:border-slate-800 bg-white"
                                                />
                                            );
                                        })}
                                    </div>
