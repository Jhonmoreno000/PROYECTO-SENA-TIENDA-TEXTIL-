...
export const getAllSellersMetrics = (sellers = [], orders = [], bugReports = []) => {
    return (sellers || []).map(vendedor => ({
        ...vendedor,
        metrics: getSellerMetrics(vendedor.id, orders, bugReports)
...
export const getAllClientsMetrics = (clients = [], orders = []) => {
    return (clients || []).map(cliente => ({
        ...cliente,
        metrics: getClientMetrics(cliente.id, orders)
...
export const segmentClients = (clients = [], orders = []) => {
    ...
    clientsWithMetrics.forEach(cliente => {
        const spent = cliente.metrics?.totalSpent || 0;
        if (spent >= 1000000) {
            vip.push(cliente);
        } else if (spent >= 300000) {
            regular.push(cliente);
        } else {
